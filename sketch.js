...
async function detectHarmfulIngredients(extractedText, allergyAlerts = []) {
  const cleaned = extractedText.toLowerCase().replace(/[^\w\s]/g, ' ').split(/\s+/).filter(Boolean);
  ...
  try {
    ...
    data.results.forEach(item => {
      if (item.found) {
        // Combine ingredient, risk sentence, and description
        foundArr.push(`${item.ingredient}: ${item.risks} ${item.description}`);
      }
    });
      ...
      Swal.fire({
        ...
        if (res.isConfirmed) {
          Swal.fire({
            ...
            html: `<pre style="text-align:left;white-space:pre-wrap">${foundArr.join('\n\n')}</pre>`
          });
        }
      });
    } else {
